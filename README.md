...
Lumino uses a captive portal for creating a connection with your home WiFi. The captive portal implementation consists of two parts, one is the WebSockets based API implemented in this `lumino-esp` project and another one is a static content stored in EEPROM and served by the web served running on the same device.
...
1. Scan available WiFi networks on your mobile phone or computer.
2. Find the one created by the device (it should have `LUMINO_` prefix) and connect to it.
...
4. Connect the device to your WiFi network by selecting its SSID in the list of the available ones. For a secured home network, which is mostly the case, you need to provide a password too.
