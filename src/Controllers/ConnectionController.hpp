...
class ConnectionController : public Services::IHttpController {
  ...
  private:
    ...
    std::unique_ptr<Core::IActionResult> onGetConnection(
        Services::IHttpRequest& request);

    std::unique_ptr<Core::IActionResult> onPostConnection(
        Services::IHttpRequest& request,
        const Core::IEntity& entity);

    std::unique_ptr<Core::IActionResult> onDeleteConnection(
        Services::IHttpRequest& request);
};
